<script setup lang="ts">
  ...
  const serviceData: any = usePage().props.services
  ...
  const deleteService = async () => {
    isModalDangerActive.value = false
    form.delete(route('services.destroy', form.id), {
      onSuccess: () => {
  ...
  const cols = ref([
    ...
    { title: 'Name', field: 'service_name', width: '200px', hide: false },
  ...
  const rows = ref(serviceData.map((service: any, index: number) => {
    return {
      sl: index + 1,
      id: service.id,
  ...
  const filteredService = computed(() => {
    if (!params.search) return serviceData.slice(0, params.pagesize);
    const query = params.search.toLowerCase();
    return serviceData?.filter((item: any) => item.service_name.toLowerCase().includes(query));
  })
</script>
...
<template>
  ...
  <Head title="Services" />
  <!-- Breadcrumb Start -->
  <BreadcrumbDefault pageTitle="Services" />
  <!-- Breadcrumb End -->
    ...
    <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <Vue3Datatable :rows="rows" :columns="cols" :sortable="true" :sortColumn="params.sort_column"
        ...
        <template #action="data">
            <template class="flex">
              <BaseButtonLink  routeName="services.edit" :routeParams="data.value.id" icon="fas fa-edit" label="Edit"
                color="info" small />
